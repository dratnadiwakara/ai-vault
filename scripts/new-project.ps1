...
.DESCRIPTION
    Creates a new project directory with the shared (cross-track) folder
    structure, copies template files from the vault, and symlinks the
    .claude/commands/ skills and agents directories back to this vault so
    all projects share one universal set of skills and agents.

    The project starts with NO tracks. After init, run scripts/new-track.ps1
    (from this vault) to scaffold the first analytical track.
...
function Copy-VaultFile {
    param([string]$RelPath, [string]$DestRelPath = $null)
    if (-not $DestRelPath) { $DestRelPath = $RelPath }
    $src = Join-Path $VaultRoot $RelPath
    $dst = Join-Path $ProjectPath $DestRelPath
...
Write-Host "Creating shared directories..."
New-ProjectDir "data\raw"
New-ProjectDir "data\constructed"
New-ProjectDir "code"
New-ProjectDir "tracks"
...
Write-Host "  4. Create your first track:"
Write-Host "       $VaultRoot\scripts\new-track.ps1 -ProjectPath '$ProjectPath' -TrackName <descriptor>"
Write-Host "  5. Initialize git: git init"
Write-Host ""
